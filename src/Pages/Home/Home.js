          ...
          {favorites.map((item, i) => (
            <Link to="/product/1" key={i} className="flex flex-col gap-3 group">
              <div className="relative aspect-[4/5] rounded-xl overflow-hidden bg-surface-light">
                {item.tag && (
                  <span
                    className={`absolute top-3 left-3 text-xs font-bold px-2 py-1 rounded-sm z-10 ${item.tag === "BEST SELLER" ? "bg-primary text-[#0d1b10]" : "bg-white/90 text-[#0d1b10] backdrop-blur-md"}`}
                  >
                    {item.tag}
                  </span>
                )}
          ...
          <div className="flex flex-col md:flex-row items-center gap-10 md:gap-20">
            ...
            <div className="w-full md:w-1/2 flex flex-col gap-6 items-start">
              ...
              <p className="text-lg text-gray-600 leading-relaxed">
                We partner directly with small-scale tea gardens in Asia to
                bring you the freshest, ethically sourced leaves. Every sip
                tells a story of tradition, craftsmanship, and respect for
                nature.
              </p>
