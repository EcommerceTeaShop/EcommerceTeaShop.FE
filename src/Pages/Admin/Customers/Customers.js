...
const Customers = () => {
  const [activeTab, setActiveTab] = useState('Danh sách người dùng');
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCustomer, setSelectedCustomer] = useState(null);
  const [selectedReview, setSelectedReview] = useState(null);

  const customers = [
    { id: 'CUS-092', name: 'Alice Green', email: 'alice@example.com', orders: 14, spent: '$345.50', status: 'Hoạt động' },
    { id: 'CUS-091', name: 'Bob Herbal', email: 'bob@herbal.com', orders: 2, spent: '$45.00', status: 'Không hoạt động' },
    { id: 'CUS-090', name: 'Charlie Chai', email: 'charlie@tea.co', orders: 28, spent: '$1,250.00', status: 'VIP' },
    { id: 'CUS-089', name: 'Diana Darjeeling', email: 'diana@outlook.com', orders: 5, spent: '$115.00', status: 'Hoạt động' }
  ];

  const reviews = [
    { id: 'REV-105', product: 'Imperial Jasmine Pearl', customer: 'Alice Green', rating: 5, text: 'Hương thơm tuyệt vời. Sẽ mua lại!', status: 'Chờ duyệt' },
    { id: 'REV-104', product: 'Classic Earl Grey', customer: 'Bob Herbal', rating: 3, text: 'Ngon nhưng hơi nhiều bergamot với khẩu vị của tôi.', status: 'Đã duyệt' },
    { id: 'REV-103', product: 'Aged Pu-erh Cake', customer: 'Charlie Chai', rating: 5, text: 'Một kiệt tác. Niên vụ 2018 rất mượt.', status: 'Chờ duyệt' }
  ];
    ...
    <div className="flex-1 overflow-y-scroll p-4 md:p-8 bg-gray-50 text-slate-900 min-h-screen relative">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm transition-opacity p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-slate-100">
            ...
            <div className="flex gap-3">
              ...
              <button 
                onClick={() => {
                  alert(`Đang mô phỏng gửi email đặt lại mật khẩu tới ${selectedCustomer.email}`);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-1">
            <p className="text-slate-500 text-sm font-medium">Điểm trung bình cửa hàng</p>
            <h3 className="text-2xl font-bold text-blue-600 mt-2">4.8 / 5.0</h3>
          </div>
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-1">
            <p className="text-slate-500 text-sm font-medium">Thành viên VIP</p>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto scrollbar-hide">
            {['Danh sách người dùng', 'Đánh giá sản phẩm'].map((tab) => (
              <button 
                key={tab}
          ...
          <div className="relative group w-full md:w-96">
            ...
            <input 
              ...
              placeholder={`Tìm ${activeTab === 'Danh sách người dùng' ? 'tên hoặc email' : 'sản phẩm hoặc đánh giá'}...`} 
              type="text"
            />
          </div>
        </div>
                    ...
                    <tr key={idx} className="group hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-4 truncate">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border ${
                          ...
                          customer.status === 'Hoạt động' ? 'bg-green-100 text-green-800 border-green-200' :
